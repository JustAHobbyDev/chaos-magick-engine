...
Enable structured output enforcement for the two response contracts. Bound the `working` block by size the way history is bounded. Decide episode pacing so that a step limit schedules a cooldown wake rather than requiring an operator. Then run the [corpus size trial](008-corpus-size-trial.md), and give the demon actual reception through feedback on Marker I.
